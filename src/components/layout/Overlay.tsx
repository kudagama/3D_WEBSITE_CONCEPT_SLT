...
import { 
  ...
  User, Phone, PlusCircle, MessageSquare, History, Lock, Unlock, AlertCircle
} from 'lucide-react';
...
export default function Overlay({ currentSection, waveParamsRef }: OverlayProps) {
  ...
  // Dashboard Sub-Tab
  const [activePortalTab, setActivePortalTab] = useState<'history' | 'file'>('history');
  ...
  const handlePortalLogin = (e: React.FormEvent) => {
    ...
    // Simple validation: Account No must be 10 digits, Mobile must be 10 digits
  ...
  const handleAddComplaint = (e: React.FormEvent) => {
    ...
    // Clear success banner after 3 seconds and switch to history list
    setTimeout(() => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', pointerEvents: 'none', zIndex: 10 }}>
      ...
      {/* ========================================================================= */}
      {/* 7. MODAL: SLT CUSTOMER PORTAL */}
      {/* ========================================================================= */}
      {showPortalModal && (
        <div className="modal-overlay">
          <div className="glass-panel modal-container" style={{ pointerEvents: 'auto', maxWidth: '650px' }}>
            
            {/* Modal Header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', borderBottom: '1px solid var(--border-color)', paddingBottom: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <Laptop size={20} className="glow-text-cyan" />
                <h3 style={{ fontSize: '20px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em' }}>SLT Customer Self-Care Portal</h3>
              </div>
              <button 
                style={{ background: 'transparent', border: 'none', color: '#fff', cursor: 'pointer' }}
                onClick={() => {
            ...
            {/* PORTAL STEP 2: DASHBOARD */}
            {portalStep === 'dashboard' && (
              <div>
                {/* Account details badge panel */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.02)', padding: '12px 16px', borderRadius: '8px', border: '1px solid var(--border-color)', marginBottom: '24px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                    <span style={{ fontSize: '10px', fontFamily: 'var(--font-geist-mono)', color: 'var(--text-muted)' }}>CONNECTED SUBSCRIBER</span>
                    <strong style={{ fontSize: '13px', color: '#fff' }}>Acct: {portalAccountNo} | Mob: {portalMobileNo}</strong>
                  </div>
                  <button 
                    onClick={handlePortalLogout}
                    className="btn-cyber" 
                    style={{ fontSize: '10px', padding: '6px 12px', borderColor: 'var(--magenta)', color: 'var(--magenta)', boxShadow: 'none' }}
                  >
                    Logout / Disconnect
                  </button>
                </div>
                ...
                <div className="sub-tabs" style={{ marginBottom: '20px' }}>
                  <button 
                    type="button"
                    className={`tab-btn ${activePortalTab === 'history' ? 'active' : ''}`}
                  ...
                  <button 
                    type="button"
                    className={`tab-btn ${activePortalTab === 'file' ? 'active' : ''}`}
                ...
                {activePortalTab === 'history' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxHeight: '40vh', overflowY: 'auto', paddingRight: '4px' }}>
                    {complaints.length === 0 ? (
                      <p style={{ color: 'var(--text-muted)', fontSize: '13px', textAlign: 'center', padding: '20px' }}>No previous tickets found.</p>
                    ) : (
                      complaints.map((item) => (
                  ...
                  <form onSubmit={handleAddComplaint} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    {showTicketSuccess && (
                      <div style={{ background: 'rgba(0, 255, 127, 0.1)', border: '1px solid #00ff7f', color: '#00ff7f', padding: '12px', borderRadius: '6px', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <Check size={16} />
                        <span>Complaint logged successfully! Ticket generated. Redirecting...</span>
                      </div>
                    )}
                    ...
                    <div>
                      ...
                      <input 
                        type="text" 
                        placeholder="e.g. Fiber line red indicator light blinking"
                    ...
                    <div>
                      ...
                      <textarea 
                        rows={4}
                        placeholder="Please describe details of connection loss, error codes, and when the issue started..."
                        value={newTicketDesc}
